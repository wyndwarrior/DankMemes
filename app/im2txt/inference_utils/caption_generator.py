...
class CaptionGenerator(object):
  ...
  def beam_search(self, sess, encoded_image, temp):
    ...
    for _ in range(self.max_caption_length - 1):
      ...
      for i, partial_caption in enumerate(partial_captions_list):
        ...
        word_probabilities /= temp
        word_probabilities = np.exp(word_probabilities)
        word_probabilities /= np.sum(word_probabilities)
        state = new_states[i]
        newwords = []
        # for beam in range(self.beam_size):
        for itr in range(100):
          w = np.random.choice(len(word_probabilities), 1, p=word_probabilities)[0]
          p = word_probabilities[w]
          if w != self.vocab.word_to_id("<UNK>") and not w in newwords:
